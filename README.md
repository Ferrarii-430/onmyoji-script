# 阴阳师脚本 (Onmyoji-Script)

仅适用于《阴阳师》桌面版游戏的自动化脚本工具。

## 使用说明

1. 下载最新发行版本的 `yys-script-x64.zip` 完整的解压到文件夹
2. 右键使用管理员身份启动 `yys-script.exe` 即可使用
3. 使用默认设置 支持游戏窗口最小化时运行脚本
4. **重要：** 必须使用管理员身份启动! 否则无法注入

## 注意事项

⚠️ **重要提醒：**
- 短暂测试目前还未被"鬼使黑",桌面版时间长 肯定会吃邮件的. 怕别用 用别怕！
- 请合理使用，遵守游戏规则
- 使用风险自负

## 更新计划

🔮 **未来版本将加入以下功能：**
1. 更隐蔽的鼠标策略
2. 全自动砸豆子
3. 任务执行途中,做无意义的事情,拟人化
4. 全自动突破 ✅
5. 全自动日常托管
6. 欢迎在issues提出


## 使用说明
🔮 **软件主要功能就是 识别->点击->等待 不断的循环步骤**
1. 选择方案->添加方案内容
![img1.png](img/img1.png)
2. 输入需要的数据 分数阈值是0-1之前的数字，建议填0.55-0.75之间，点击开始截图选择需要识别的图片 ok保存即可
![img2.png](img/img2.png)
![img3.png](img/img3.png)
3. 设置循环次数 次数为0时是无限循环 点击启动即可开始执行脚本逻辑
![img4.png](img/img4.png)
4. 中途可以加入是否有勾协的判断，防止被鬼使黑

## 脚本逻辑完全依靠自主设置 自由发挥 每次爬塔或者周年庆999只需要简单的截图设置下 即可完成，解放双手

---

## 更新日志
🔮 **v1.5**
# 1.5版本更新
## 一. 界面优化 更加美观

# 二. bug修复
1.修复获取DX11数据的线程安全异常，可能导致画面花屏
2.截图异常
3.多种异常的闪退
4.运行过程中线程假死
5.无法停止任务

# 三. 添加了全定义YOLO标签识别

# 四. 优化了全自动突破，目前基本可用

# 五. OCR识别添加了区域限制

🔮 **v1.4**
1. 添加yolo v5 识别，模型来自 https://github.com/shuiyuewuyan1/yolo-games-weights 感谢🙏大佬
2. 添加全自动突破功能

🔮 **v1.3**
1. 使用DLL获取游戏截屏
2. 添加ocr识别功能 鸣谢@RapidOCR

*本工具仅供学习交流使用，请勿用于商业用途。*