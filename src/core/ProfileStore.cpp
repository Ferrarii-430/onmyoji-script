            ...
            for (int j = 0; j < stepsArray.size(); ++j) {
                ...
                if (stepsObj["stepsId"].toString() == stepsId) {
                    ...
                    if (stepsObj.contains("imagePath") && !stepsObj["imagePath"].isNull()) {
                        ...
                        QString savePath = AppPaths::instance().screenshotPath() + imagePathToDelete;
                        // 删除图片文件
...
void refreshConfig()
{
    QFile file(AppPaths::instance().configPath());
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        Logger::log(QString("无法打开配置文件：" + file.errorString() + "路径:" + AppPaths::instance().configPath()));
        return;
    }
